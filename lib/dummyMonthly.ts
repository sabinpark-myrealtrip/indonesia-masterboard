...
function sumMetrics(list: MonthlyMetrics[]): MonthlyMetrics {
  const s = list.reduce((acc, m) => ({
    ...
    cmr: 0, cfr: 0, cvr: 0,
  }), { rsv:0, crsv:0, gmv:0, cgmv:0, cm:0, cmr:0, cfr:0, uv:0, purchaseCompleteUv:0, cvr:0 });
...
export function generateMonthlyDummy(currentMonth: string): MonthlyData {
  ...
  const rows: CityMonthRow[] = CITIES.map(city => {
    ...
    // 월별 성장 트렌드 (약 5~10% MoM 성장)
    const monthScale = (mo: string) => {
      const idx = months.indexOf(mo);
      return 0.75 + idx * 0.05;
    };

    const partners: PartnerMonthRow[] = PARTNERS.map(partner => ({
      partner,
      months: Object.fromEntries(
        months.map(mo => [mo, makeMetrics(base * partnerShare[partner], monthScale(mo))])
      ),
    }));

    const total: Record<string, MonthlyMetrics> = Object.fromEntries(
      months.map(mo => [mo, sumMetrics(partners.map(p => p.months[mo]))])
    );

    return { city, total, partners };
  });
  ...
  const totalRow: CityMonthRow = {
    ...
    total: Object.fromEntries(
      months.map(mo => [mo, sumMetrics(rows.map(r => r.total[mo]))])
    ),
    partners: PARTNERS.map(partner => ({
      partner,
      months: Object.fromEntries(
        months.map(mo => [mo, sumMetrics(rows.map(r => r.partners.find(p => p.partner === partner)!.months[mo]))])
      ),
    })),
  };

  return { months, rows: [totalRow, ...rows] };
}
